...
router.post('/add', authMiddleware, async (req, res) => {
	try {
		...
		if (examExists) {
			return res.status(200).send({ message: 'Imtihon allaqachon mavjud', success: false });
		}
		req.body.questions = [];
		const newExam = new Exam(req.body);
		await newExam.save();
		res.send({
			message: 'Imtihon muvaffaqiyatli qoʼshildi',
			success: true,
		});
	} catch (error) {
		res.status(500).send({
...
router.post('/get-all-exams', authMiddleware, async (req, res) => {
	try {
		...
		res.send({
			message: 'Imtihonlar muvaffaqiyatli olindi',
			data: exams,
			success: true,
		});
	} catch (error) {
...
router.post('/get-exam-by-id', authMiddleware, async (req, res) => {
	try {
		...
		res.send({
			message: 'Imtihon muvaffaqiyatli olindi',
			data: exam,
			success: true,
		});
	} catch (error) {
...
router.post('/delete-exam-by-id', authMiddleware, async (req, res) => {
	try {
		...
		res.send({
			message: 'Imtihon muvaffaqiyatli oʻchirildi',
			success: true,
		});
	} catch (error) {
		res.status(500).send({
...
router.post('/add-question-to-exam', authMiddleware, async (req, res) => {
	try {
		...
		await exam.save();
		res.send({
			message: 'Savol muvaffaqiyatli qoʻshildi',
			success: true,
		});
	} catch (error) {
		res.status(500).send({
...
router.post('/delete-question-in-exam', authMiddleware, async (req, res) => {
	try {
		...
		await exam.save();
		res.send({
			message: 'Savol muvaffaqiyatli oʻchirildi',
			success: true,
		});
	} catch (error) {
		res.status(500).send({
